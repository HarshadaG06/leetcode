...
public class kidsWithGreatestNoOfCandies {
    static int max = 0;
    public static List<Boolean> helper(int[] candies, int size, int extraCandies){
        if(size == -1){
        ...
        List<Boolean> result = helper(candies, size-1, extraCandies);
        int totalCandies = candies[size] + extraCandies;
        if(totalCandies >= max){
            result.add(true);
        }else{
            result.add(false);
    ...
    public static void main(String[] args) {
        ...
        int[] candies = {2, 8, 7};
        int extraCandies = 1;
        List<Boolean> result = helper(candies, candies.length-1, extraCandies);
